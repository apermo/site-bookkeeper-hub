...
use Apermo\SiteBookkeeperHub\Handler\CategoriesHandler;
...
use Apermo\SiteBookkeeperHub\Handler\SiteDeleteHandler;
use Apermo\SiteBookkeeperHub\Handler\SiteHandler;
use Apermo\SiteBookkeeperHub\Handler\SitePatchHandler;
use Apermo\SiteBookkeeperHub\Handler\SitesHandler;
use Apermo\SiteBookkeeperHub\Handler\ThemesHandler;
use Apermo\SiteBookkeeperHub\Handler\UsersHandler;
use Apermo\SiteBookkeeperHub\Handler\VulnerabilityStatusHandler;
...
$category_repo = new CategoryRepository( $database );

$report_handler = new ReportHandler( $repo, $token_auth, $network_auth, $accepted_envs, $category_repo, $vuln_manager );
$network_report_handler = new NetworkReportHandler( $network_repo, $network_auth );
$sites_handler = new SitesHandler( $repo, $client_auth, $stale_hours, $category_repo );
...
$site_delete_handler = new SiteDeleteHandler( $repo, $client_auth );
$site_patch_handler = new SitePatchHandler( $repo, $client_auth );
$categories_handler = new CategoriesHandler( $category_repo, $client_auth );
$users_handler = new UsersHandler( $repo, $client_auth );
$vuln_status_handler = new VulnerabilityStatusHandler( $client_auth, $vuln_manager, $vuln_repo );
...
$router->patch( '/sites/{id}', [ $site_patch_handler, 'handle' ] );
$router->delete( '/sites/{id}', [ $site_delete_handler, 'handle' ] );
$router->get( '/categories', [ $categories_handler, 'handleList' ] );
$router->post( '/categories', [ $categories_handler, 'handleCreate' ] );
$router->put( '/categories/{id}', [ $categories_handler, 'handleUpdate' ] );
$router->delete( '/categories/{id}', [ $categories_handler, 'handleDelete' ] );
$router->get( '/plugins', [ $plugins_handler, 'handle' ] );
$router->get( '/themes', [ $themes_handler, 'handle' ] );
$router->get( '/users', [ $users_handler, 'handle' ] );
$router->get( '/vulnerability-status', [ $vuln_status_handler, 'handle' ] );
